...
const Cart = () => {
  ...
  const handleOnSubmit = (e) => {
    ...
    const products = cart.map((item) => {
      const toppings = item.toppings.map((topping) => topping.id);
      return { idProduct: item.id, qty: 1, toppings: toppings };
    });
    ...
    const postTransaction = API.post("/transaction", formData);

    const patchPromises = cart.flatMap((item) => {
      const stock = item.quantity;
      return [
        API.patch("/productStock/" + item.id, {
          stock,
        }),
        API.patch("/toppingStock/" + item.id, {
          stock,
        }),
      ];
    });

    Promise.all([postTransaction, ...patchPromises])
  ...
  return (
    <Container>
      <Row className="justify-content-md-center content-cart-user mt-1">
        <Col md={10}>
          ...
          <div className="d-flex flex-column justify-content-space-beetwen mt-3">
            <div>
              <Form onSubmit={handleOnSubmit}>
                ...
                <Form.Group className="formGroup">
                  <Form.Label>Note Pembelian</Form.Label>
                  <Form.Control
                    name="address"
                    className="carts-input"
                    onChange={(e) => handleChange(e)}
                    type="text"
                    placeholder="Note Pembelian"
                    as="textarea"
                    rows={3}
                    required
                  ></Form.Control>
